...
# welcome page
# View 함수 -> 1개 이상 파라미터 선언 (HttpRequest 객체를 받는 파라미터)
...
def welcome(request):
    ...
    res = render(
        request, # HTTPRequest (HTTP 요청 정보를 담은 객체)
        "polls/welcome.html", # 호출할 template 파일 경로
        {"current":now, "name":"홍길동"} # context value: view가 template에 전달하는 값들
...
def list(request):
    """
    paging 처리 view
    - 요청 파라미터로 page 번호를 받는다
    - context data (Template에게 전달하는 값)
        - 현재 페이지의 데이터
        - 현재 페이지가 속한 페이지 그룹의 시작 페이지 번호, 끝 페이지 번호
        - 현재 페이지 그룹의 시작 페이지가 이전 페이지가 있는지 여부/ 이전 페이지 번호
        - 현재 페이지 그룹의 끝 페이지가 다음 페이지가 있는지 여부/ 다음 페이지 번호        
    """
    paginate_by = 10  # 한 페이지 당 데이터 개수(정하기 나름)
    ...
    # 현재 페이지가 속한 페이지 그룹의 시작/끝 페이지 번호 조회
    ...
    # 그룹의 시작 페이지의 이전 페이지 유무/ 있다면 페이지 번호
    # 그룹의 끝 페이지의 다음 페이지 유무/ 있다면 페이지 번호
    ## 시작 페이지
...
## http://url:port/vote_form?pk=3 => 요청 파라미터(QueryString)
...
## 요청URL: /polls/vote
## view: vote
## 응답: - 정상처리: vote_result view를 호출 (redirect방식)
##       - 요청 파라미터 검증 실패 (선택 안하고 투표) -> polls/vote_form.html
# View에서 요청 파라미터 조회
## GET방식 요청: request.GET['요청 파라미터 이름'], request.GET.get['요청 파라미터 이름']
## POST방식 요청: request.POST['요청 파라미터 이름'], request.POST.get['요청 파라미터 이름']
# []: 요청 파라미터 이름으로 넘어온게 없으면 Exception 발생
# get: 요청 파라미터 이름으로 넘어온게 없으면 None 반환

def vote(request):
    # post 요청 파라미터 - name: choice
    ...
    if choice_id is not None:
        ...
        # 투표 결과 페이지로 이동 -> view를 호출
        # url = f"/polls/vote_result/{question_id}" # view의 url
        url = reverse("polls:vote_result", args=[question_id])
        print(url)
        return redirect(url)
        # redirect(url): Redirect 방식 응답. Web Browser가 지정한 url로 다시 요청하도록 응답

    else: # choice 요청 파라미터가 없는 경우
        question = Question.objects.get(pk=question_id)
        return render(
            request,
...
##     - HOST: list view를 호출 => redirect() 이동 (새로고침 해도 추가되지 않도록)
...
def vote_create(request):
    ...
    if http_method == "GET":
        ...
        # 같은 이름으로 여러개 값이 넘어온 경우(choice_text=aaa&choice_text=bbb&)
        # request.GET/POST.getlist('요청 파라미터 이름): list
